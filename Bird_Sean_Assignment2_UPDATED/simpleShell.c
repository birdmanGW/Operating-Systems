...
int ampersand = 0, chk = 0;          //Hold value if ampersand is present
...
int main(void)
{
    ...
    while (should_run) 
    {
        ...
        else if (strcmp(cmd, "!!") == 0) 
        {
            ...
            if (strcmp(hist[0], "!!") == 0 && chk == 0) {
                printf("No recent history command to execute. \n");
                chk = 1;
            }

            //else execute mostrecenthist
            else
...
{
    //temp array to parse hist[num]
    char *newArray[MAX_LINE];

    //Check for ampersand, if found, change the value of ampersand to 1
    if (strchr(hist[num], '&') != NULL) 
    {
        ampersand = 1;
        newArray[0][strlen(hist[num])-1] = 0;
    }

    //parse delimiting spaces (if any) and add to args
    parseCmd(hist[num], newArray);
